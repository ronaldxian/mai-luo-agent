...
public sealed class OpenCodeService
{
    ...
    public OpenCodeService(AppSettings settings)
    {
        var configuredPath = settings.OpenCode.CliPath;
    ...
    public async Task<IReadOnlyList<SessionInfo>> ListSessionsAsync(CancellationToken ct = default)
    {
        ...
        try
        {
            var sessions = JsonSerializer.Deserialize(output, OcCliJsonContext.Default.ListSessionInfo);
