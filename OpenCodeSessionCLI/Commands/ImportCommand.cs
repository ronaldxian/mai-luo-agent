...
public static class ImportCommand
{
    public static async Task<int> ExecuteAsync(string[] args)
    {
        ...
        if (string.IsNullOrWhiteSpace(sessionId))
        {
            ...
            AnsiConsole.Info("[INFO] Usage: oc-cli import <sessionId>[/]");
            return ExitCodes.GeneralFailure;
        }

        AnsiConsole.Info("[INFO] Starting import for session: {0}[/]", sessionId);
        ...
        try
        {
            ...
            AnsiConsole.Info("[INFO] Downloading session from Gist...[/]");

            var result = await syncService.DownloadAndImportAsync(sessionId);
        ...
        {
            Log.Logger.Error(ex, "Import command failed");
